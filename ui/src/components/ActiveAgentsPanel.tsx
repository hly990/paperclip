    ...
    <div>
      ...
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">{t("panel.noRecentRuns")}</p>
        </div>
      ) : (
    ...
    <div className={cn(
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive ? t("panel.liveNow") : run.finishedAt ? t("panel.finished", { time: relativeTime(run.finishedAt) }) : t("panel.started", { time: relativeTime(run.createdAt) })}</span>
            </div>
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <RunTranscriptView
          ...
          emptyMessage={hasOutput ? t("panel.waitingTranscript") : isActive ? t("panel.waitingOutput") : t("panel.noTranscript")}
        />
      </div>
    </div>
  );

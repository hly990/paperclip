...
export function RunActivityChart({ runs }: { runs: HeartbeatRun[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("component.charts.noRunsYet")}</p>;
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("component.charts.noIssues")}</p>;
...
const statusLabelKeys: Record<string, string> = {
  todo: "component.charts.statusTodo",
  in_progress: "component.charts.statusInProgress",
  in_review: "component.charts.statusInReview",
  done: "component.charts.statusDone",
  blocked: "component.charts.statusBlocked",
  cancelled: "component.charts.statusCancelled",
  backlog: "component.charts.statusBacklog",
};
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "#6b7280", label: t(statusLabelKeys[s] ?? s) }))} />
    </div>
  );
}

...
export function Agents() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Bot} message={t("page.agents.selectCompany")} />;
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <Tabs value={tab} onValueChange={(v) => navigate(`/agents/${v}`)}>
          <PageTabBar
            items={[
              ...
              { value: "active", label: t("page.agents.active") },
              { value: "paused", label: t("page.agents.paused") },
              { value: "error", label: t("page.agents.error") },
            ]}
            value={tab}
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{t("page.agents.agentCount", { count: filtered.length })}</p>
      )}
      ...
      {agents && agents.length === 0 && (
        <EmptyState
          icon={Bot}
          message={t("page.agents.createFirst")}
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
        {t("page.agents.live")}{liveCount > 1 ? ` (${liveCount})` : ""}
      </span>
    </Link>
  );
}

...
export function NotFoundPage({ scope = "global", requestedPrefix }: NotFoundPageProps) {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("page.notFound.breadcrumb") }]);
  }, [setBreadcrumbs, t]);
  ...
  const title = scope === "invalid_company_prefix" ? t("page.notFound.companyNotFound") : t("page.notFound.pageNotFound");
  const description =
    scope === "invalid_company_prefix"
      ? t("page.notFound.noCompanyMatchesPrefix", { prefix: normalizedPrefix ?? "unknown" })
      : t("page.notFound.routeDoesNotExist");
    ...
    <div className="mx-auto max-w-2xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4 rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("page.notFound.requestedPath")}: <code className="font-mono">{currentPath}</code>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Button asChild>
            <Link to={dashboardHref}>
              ...
              {t("page.notFound.openDashboard")}
            </Link>
          </Button>
          <Button variant="outline" asChild>
            <Link to="/">{t("page.notFound.goHome")}</Link>
          </Button>
        </div>
      </div>
    </div>

...
export function Routines() {
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("page.routines.routineCreated"),
        body: t("page.routines.addFirstTrigger"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("page.routines.failedToUpdate"),
        body: mutationError instanceof Error ? mutationError.message : t("page.routines.couldNotUpdate"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("page.routines.runFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("page.routines.couldNotStartRun"),
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("page.routines.selectCompany")} />;
  }
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("page.routines.newRoutine")}</p>
              <p className="text-sm text-muted-foreground">
                {t("page.routines.defineRecurringWork")}
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={t("page.routines.routineTitle")}
                rows={1}
                value={draft.title}
                onChange={(event) => {
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("page.routines.advancedSettings")}</p>
                    <p className="text-sm text-muted-foreground">{t("page.routines.advancedSettingsHint")}</p>
                  </div>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("page.routines.creating") : t("page.routines.createRoutine")}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("page.routines.failedToCreate")}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : t("page.routines.failedToLoad")}
          </CardContent>
        </Card>
      ) : null}
      ...
      <div>
        {(routines ?? []).length === 0 ? (
          <div className="py-12">
            <EmptyState
              icon={Repeat}
              message={t("page.routines.noRoutines")}
            />
          </div>
        ) : (
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-muted-foreground border-b border-border">
                  ...
                  <th className="px-3 py-2 font-medium">{t("page.routines.project")}</th>
                  <th className="px-3 py-2 font-medium">{t("page.routines.agent")}</th>
                  <th className="px-3 py-2 font-medium">{t("page.routines.lastRun")}</th>
                  <th className="px-3 py-2 font-medium">{t("page.routines.enabled")}</th>
                      ...
                      <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {isArchived ? t("page.routines.archived") : enabled ? t("page.routines.on") : t("page.routines.off")}
                          </span>
                        </div>
                      </td>
                      <td className="px-3 py-2.5 text-right" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            >
                              {runningRoutineId === routine.id ? t("page.routines.running") : t("page.routines.runNow")}
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem
                              onClick={() =>
                              ...
                              disabled={isStatusPending || isArchived}
                            >
                              {enabled ? t("page.routines.pause") : t("page.routines.enable")}
                            </DropdownMenuItem>
                            <DropdownMenuItem
                              onClick={() =>
                                updateRoutineStatus.mutate({
                            ...
                            >
                              {routine.status === "archived" ? t("page.routines.restore") : t("page.routines.archive")}
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </td>
